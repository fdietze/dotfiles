  ...
  lib.mkIf (desktop == "noctalia-niri") {
    ...
    home.file."bin/noctalia-gtk-theme" = {
      ...
      source = pkgs.writeShellScript "noctalia-gtk-theme" ''
        ...
        mode="''${1:-}"
        if [ -z "$mode" ]; then
          # Seed path: no arg, so derive from settings.json. This is only used
          # outside a live toggle (where darkMode is not yet stale).
          settings="$HOME/.config/noctalia/settings.json"
          if [ -f "$settings" ] \
            && [ "$(${pkgs.jq}/bin/jq -r '.colorSchemes.darkMode' "$settings")" = "true" ]; then
            mode=dark
          else
            mode=light
          fi
