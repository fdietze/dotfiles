{lib}: let
  ...
  specialisationName = {
    ...
    if hasThemeVariants desktop
    then "${desktop}-${theme}"
    else desktop;

  switchToConfigurationCommand = name: "/nix/var/nix/profiles/system/specialisation/${name}/bin/switch-to-configuration switch";
  ...
  mkDesktopSpecialisation = {
    ...
    myConfig =
      ...
      else {my.desktop = lib.mkForce desktop;};
  in
    lib.nameValuePair name {
      configuration = myConfig // extraConfig;
    };

  mkThemedSpecialisation = desktop: theme:
    mkDesktopSpecialisation {
